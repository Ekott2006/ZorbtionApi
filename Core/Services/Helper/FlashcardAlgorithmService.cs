...
public class FlashcardAlgorithmService: IFlashcardAlgorithmService
{
    ...
    public FlashcardResult Calculate(int quality, CardState currentState, int interval, double easeFactor, int stepIndex)
    {
    ...
    private static FlashcardResult HandleNewOrLearning(int quality, double easeFactor, int stepIndex)
    {
        switch (quality)
        {
            ...
            case < 3:
                return new FlashcardResult { Interval = LearningSteps[0], EaseFactor = easeFactor, State = CardState.Learning, LearningStepIndex = 0 };
            // "Easy" - Skip learning and graduate immediately
            case 5:
                return new FlashcardResult { Interval = EasyInterval, EaseFactor = easeFactor, State = CardState.Review, Repetitions = 1 };
        }

        // "Good" - Advance to next step or graduate
        int nextStep = stepIndex + 1;
        return nextStep >= LearningSteps.Length ? new FlashcardResult { Interval = GraduationInterval, EaseFactor = easeFactor, State = CardState.Review, Repetitions = 1 } : new FlashcardResult { Interval = LearningSteps[nextStep], EaseFactor = easeFactor, State = CardState.Learning, LearningStepIndex = nextStep };
    }
    ...
    private static FlashcardResult HandleReview(int quality, int interval, double easeFactor)
    {
        ...
        if (quality < 3) // LAPSE: The user forgot a mature card
        {
            ...
            return new FlashcardResult { Interval = LearningSteps[0] * dayToMinute, EaseFactor = newEase, State = CardState.Relearning, LearningStepIndex = 0 };
        }
        ...
        return new FlashcardResult { Interval = nextInterval * dayToMinute, EaseFactor = nextEaseFactor, State = CardState.Review };
    }
}